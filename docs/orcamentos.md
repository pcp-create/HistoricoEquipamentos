...
O responsável é atribuído automaticamente ao salvar, pelo Display name do usuário autenticado no Supabase (ou e-mail quando não há nome). Não há campo de responsável para preenchimento manual. Ao salvar novamente, o responsável passa a ser quem realizou esse salvamento; os e-mails de autoria continuam registrados.
