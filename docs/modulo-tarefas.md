...
Listas geral e pessoal, busca, filtro de status e calendário mensal baseado no vencimento do processo. Sem data aparece em bloco próprio. Números TAR-… abrem painel lateral direito (Escape fecha). Links na situação do equipamento e no plano mostram ocorrências atuais e anteriores. Notas têm título, descrição, autor e data, em ordem cronológica inversa. Anexos até 3 MB, máximo 20 por tarefa; armazenados no banco com acesso autenticado e download forçado, nunca publicados em bucket aberto. Não há remoção de histórico nesta versão.
